...
class YRMGetTestCase(MockBoto3TestCase):

    # _yrm_get() is mocked out in test_emr_pooling.py; this make sure that
    # it gets called in a way that makes sense.

    def setUp(self):
        super(YRMGetTestCase, self).setUp()

        key_pair_file = self.makefile('EMR.pem')

        self.runner = self.make_runner(
            '--ec2-key-pair', 'EMR',
            '--ec2-key-pair-file', key_pair_file,
        )
        self.runner.run()

        self._ssh_run = self.start(
            patch.object(
                self.runner.fs.ssh, '_ssh_run',
                return_value=(b'{"foo": "bar"}', b'')
            )
        )

    def test_path_alone(self):
        self.assertEqual(
            self.runner._yrm_get('metrics'),
            dict(foo='bar'),
        )

        host = self.runner._address_of_master()

        self._ssh_run.assert_called_once_with(
            host, [
                'curl', '-fsS', '-m', '20',
                'http://%s:8088/ws/v1/cluster/metrics' % host,
            ],
        )

    def test_host(self):
        self.assertEqual(
            self.runner._yrm_get('metrics', host='heavenly'),
            dict(foo='bar'),
        )
        ...
        self._ssh_run.assert_called_once_with(
            'heavenly', [
                'curl', '-fsS', '-m', '20',
                'http://heavenly:8088/ws/v1/cluster/metrics',
            ],
        )

    def test_port(self):
        self.assertEqual(
            self.runner._yrm_get('metrics', host='heavenly', port=12345),
            dict(foo='bar'),
        )
        ...
        self._ssh_run.assert_called_once_with(
            'heavenly', [
                'curl', '-fsS', '-m', '20',
                'http://heavenly:12345/ws/v1/cluster/metrics',
            ],
        )

    def test_timeout(self):
        self.assertEqual(
            self.runner._yrm_get('metrics', host='heavenly', timeout=60),
            dict(foo='bar'),
        )
